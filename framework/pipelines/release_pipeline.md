...
### Stage R8: [Handoff to Orchestrator: Phase 6 Final Release Approval Gate] 🛑
*Planning Constraint:* Do not execute final release. Yield control back to `ORCHESTRATOR.md` to enforce the global LLM Hard Stop. Await explicit User Authorization.
...
| Final Sign-off | R8 | User reviews Unified Delivery Report | [Orchestrator Handoff] |

The Final Sign-off at Stage R8 is the only user-facing gate in this pipeline, enforced by the Orchestrator. Nothing is deployed without explicit user authorization.
