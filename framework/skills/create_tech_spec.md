...
3. Adaptive Architecture Selection (Lightweight, Modular, or Enterprise Feature-Based folder structure and rationale)
...
5. **Adaptive Architecture Selection.** Before generating the folder structure, analyze the Project Type, Scope Classification, Complexity, and Expected Scalability. Use the Adaptive Architecture Decision Matrix to automatically choose one architecture:
   - **Lightweight Feature-Based:** (Landing Pages) `src/app/`, `src/features/`, `src/shared/`, `src/assets/`, `src/styles/`
   - **Modular Feature-Based:** (Medium SaaS) Adds `src/services/`, `src/hooks/`
   - **Enterprise Feature-Based:** (Large CRMs/E-commerce) Adds `src/providers/`, `src/routes/`, `src/config/`, `src/lib/`, `src/store/`
   Do not hardcode a single folder structure. Fully explain the generated folder structure and why it was chosen.

6. **Design the component hierarchy.** Map out how components will nest and relate to each other. Identify shared and leaf components.

7. **Identify reuse opportunities.** For every component needed, check the Application Analysis Report first. Only plan new components if no suitable existing component can be used.

8. **Define files to create.** List every new file with its exact path relative to the project root, the component or module name, and a one-line description of its purpose.

9. **Define files to modify.** List every existing file that must be changed, with the specific modification described (e.g., "Add new route entry," "Import new component," "Extend existing utility function").

10. **Plan state management.** Define which state is local to a component versus global. Specify the state management approach (context, store, signals, etc.) aligned with the project's existing pattern.

11. **Define design token requirements.** List every token (color, spacing, typography) the new components will use. Reference the project's Design System.

12. **Identify API integration points.** If the feature requires data fetching or mutation, define the endpoint, HTTP method, request payload, and expected response structure.

13. **Define implementation order.** Number the implementation tasks in dependency order — base utilities first, then shared components, then feature-specific components, then page assembly.
...
14. **Write the Risk Assessment.** Identify anything that could go wrong during implementation and how to mitigate it.

15. **Self-review.** Verify the Tech Spec against the Validation Checklist before submission.

16. **Submit.** Deliver the Tech Spec to the Orchestrator and halt. Await user approval.
...
- [ ] Adaptive Architecture Selection and Naming Conventions are explicitly defined and justified based on complexity.
